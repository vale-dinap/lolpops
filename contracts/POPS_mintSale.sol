...
contract POPSsale is Ownable, Pausable, ReentrancyGuard, Whitelist, MintingRandomizer{
    ...
    uint8 whitelistValidity = 4;                                                                          // In days
    ...
    modifier useWhitelist (address buyer, uint256 amount){
        if(block.timestamp < (saleStart + (1 days * whitelistValidity) )){
            require(amount < 1 + getWhitelistAllowance(buyer) + nonReserved(), "Request exceeds allowance (whitelist + non_reserved)");
            _useWhitelistAllowance(buyer, amount);
        }
        _;
    }
    ...
    function addToWhitelist(address[] calldata _addresses) public onlyOwner returns(bool success){        // use it are already derived from the base contract
    ...
    function nonReserved() view private returns(uint256 amount){
        ...
        if(block.timestamp < (saleStart + (1 days * whitelistValidity))){ amount  -= getWhitelistTotalAllowance();}
    }
    // Check if currently during golden tickets validity
    ...
    // Main buy function
    function buy(uint8 _amount, bool useGT) payable public onlyDuringSale whenNotPaused nonReentrant useWhitelist(msg.sender, _amount) returns(bool success){
        ...
        uint8 credits;
        if (useGT) credits = useGoldenTickets(msg.sender, _amount);                                       // Spend golden tickets
